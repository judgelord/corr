...
In 2023, Columbia had the highest Gini coefficient at .55. 
...
- Table: `tables/models_total_con.tex` (sans stars)
...
modelsummary::modelsummary(models_total_con, stars = T, 
             add_rows = rows, 
             coef_map = cm,
             gof_map = gm, 
             coef_omit = coef_omit) %>% 
...
### Total + Constituency Service Narrative 
 ...
 `@tab-models_total_con` shows the effects of institutional power and expierence on overall requests to federal agencies (Models 1 and 1) and constituency service requests (Models 3 and 4). Models 3 and 4 are identical to models 1 and 2 except that we subset the data to only legislator requests hand-coded as constituency service. 
Columns 2 and 4 of `@tab-models_total_con` provides the estimated effects from the difference-in-differences specification in `@eq-diff1`. 
