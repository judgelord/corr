...
dcounts_tenure %>% left_join(members %>% distinct(bioname, icpsr)) %>%
  ...
  summarise(per_agency = perYear) |> arrange(-per_year_per_agency) |> 
  kablebox()
...
For predictions, we need an average member-year-agency triad. The first table below shows the members who averaged  `r perYear_mean2`, entered Congress between 2007 and 2011, were observed in our data for at least seven years, and most often submitted exactly one letter to an agency (the closest integer  `r perYear_mean2` ). For realism, this person would ideally, at some point in our data, become a chair. Of the modal legislators, John Barrasso meets this last criterion. The IRS is a fairly modal agency and one of the agencies to which John Barrasso submitted one letter in 2007, so predictions are at John Barrasso X IRS X 2007. Other legislators, years, and agencies are simply intercept shifts. 
...
#TODO deal with chamber switchers 
...
party_crosswalk <- members  %>% 
  ...
  left_join(distinct(dcounts_per_district, icpsr, chamber, congress, state_dist, new_member)) %>%
  ...
  distinct(congress, state_dist,  state_dist_party, new_member) %>% 
...
party_crosswalk  |> #filter(chamber == "House") |> 
  select(state_dist, congress, state_dist_party, lag_p, same_party, new_member) # |> # view()

# make variable for prior seat holder party WITHIOUT REDISTRICTING 
party_crosswalk2 <- members  %>% 
  mutate(state_dist = paste(state, district_code, sep = "_")) %>% # NOTICE THIS IS DIFFERENT 
  ungroup() %>% 
  # add new_member #FIXME WHY IS THIS INTRODUCING NAs?
  left_join(distinct(dcounts_per_district, icpsr, chamber, congress, new_member)) %>%
  # # for senate, capture mixed delegations 
  group_by(state_dist, congress) %>% 
  arrange(party_name) %>% 
  mutate(state_dist_party = unique(party_name) |> paste(collapse = ";")) %>% 
...
#FIXME WHY NAs?
party_crosswalk2  |> #filter(chamber == "House") |> 
  select(state_dist, congress, state_dist_party, lag_p, same_party, new_member) # |> # view()


#FIXME WHAT ABOUT SENATE? 

members |> count(is.na(new_member))
members |> count(is.na(same_party))
...
dcounts_per_district  %>% count(is.na(same_party))

dcounts_per_district |> #filter(chamber == "House") |> 
  select(icpsr, state_dist, congress, state_dist_party, lag_p, same_party, new_member) # |> view()




dcounts_per_district |> count(same_party)
```
...
```{r m-district,  fig.show="hold", out.width="50%", fig.height=4.5, fig.cap= "district", fig.subcap= "c(1,2)"}
dcounts_per_district %<>% 
  mutate(Legislator = icpsr,
         Year = year) %>% 
  filter(year >2006 & year < 2019)
...
coefplot(m_district_cross, horiz = T, drop = "(Intercept)") 
...
coefplot(m_district_cross_party, horiz = T, drop = "(Intercept)") 
...
if(testing){modelsummary(m_district_dnd, gof_omit = "R.*", coef_omit = coef_omit)}

coefplot(m_district_dnd, horiz = T) 
...
coefplot(m_district_dnd_party, horiz = T) 
```
...
We estimate that becoming a committee chair causes the district of constituency service to policy work to decrease by `r -beta$chair` (95-percent confidence interval [`r beta$chair + se$chair*1.96` , `r beta$chair - se$chair*1.96` ]). Becoming a ranking member of a committee causes a decrease of `r -beta$ranking_minority`  in the district.
...
```{r m-district-predicted, fig.show="hold", out.width="50%", fig.cap= "district", fig.subcap= c("1","2","3","4")}
...
predicted %>%
ggchair() +
  labs(title = "Predicted district of Constituent to Policy Work\n(Cross-Sectional)",
       x = "",
       y = "Predicted district of\nConstituent to Policy Work",
       fill = "",
       color = "",
       shape = "")
