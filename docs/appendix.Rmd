...
# Data  
```{r, eval=TRUE}
read_csv(here("data/_FOIA_response_table.csv")) %>% select(-ends_with("1")) %>% kablebox_long()
```
...
dcounts %>% 
  ...
  geom_text(aes(x = Percentile, y = mean, label = ifelse(rank <14 | Percentile < 30, member_state, "")), check_overlap = T, size = 2, hjust = "inward") + 
...
- [ ] members.Rdata going back to most senior member's first Congress
- [ ] what to do with chamber switchers?
