...
### Without agency-year FE:

```{r m-total-without-agency-year-FE,  fig.show="hold", out.width="50%", fig.height=4.5, fig.cap= "Total Letters", fig.subcap= ""}
testing = F
...
### Total Letters Table (without agency-year FE)
...
rows <- tibble(
  term = c("Dependent Variable", "All Legislators", "Served At Least Second Term"),
  `1` = c("Count", "✓", ""),
  `2` =c("Count", "✓", ""),
  `3`  = c("Count", "", "✓"),
  `4` = c("Log(Count+1)", "✓", "") #"✓"
...
m <- modelsummary(models_total, stars = T, 
             add_rows = rows, 
             coef_map = cm,
             gof_omit = "R.*|AIC|BIC|Log.*",
             coef_omit = "(Intercept)", output = "html") 
...
modelsummary(models_total, # stars = T, 
             ...
             coef_omit = "(Intercept)",
             output = "latex") %>%
  str_replace("FE: Legislator_x_Agency", "Legislator x Agency Fixed Effects") %>% 
  str_replace("FE: Year_x_Agency", "Year x Agency Fixed Effects") %>% 
  str_replace_all("X", "✓") %>% 
...
m_total_dnd <-feglm (perYear ~ 
                       chair + ranking_minority + prestige + 
                    first + second + third + fourth + fifth + sixth + 
                    majority + presidents_party | Legislator_x_Agency + Year_x_Agency, 
                    cluster = "Legislator",
                    # ALT vcov = hetero ~ ssc(cluster.adj = TRUE),
           data = dcounts_tenure)
...
### Total Letters Table (with agency-year FE)
