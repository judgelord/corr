...
To assess how institutional power shapes behavior, we assemble a massive new database of \input{tables/n} legislator requests to a near census of departments, agencies, and sub-agencies between 2007 and 2023.
Leveraging variation within legislator-agency pairs, we find that legislators prioritize policy work as they gain power but simultaneously maintain their levels of constituency service.
Leveraging variation within district-agency pairs, we find that new legislators do less policy work textit{and} constituency service than more senior legislators.
...
Word Count: 10523 % abstract = 150 (as of 4-24-2024) 
...
\input{tables/models_total.tex} % this one is from replication.rmd
\end{center}
...
Model 1 (the first column of Table \ref{t:models_total}) shows this is true in a cross-sectional comparison across legislators. Model 1 excludes the legislator-agency and year-agency fixed effects, but it does include controls for majority status and being from the same party as the president. 
...
To address potential confounding in across-legislator comparisons, the estimates from Model 2 (Column 2 of Table \ref{t:models_total}) provide the estimated effects from the difference-in-differences specification in Equation \ref{e:diff1}. Across all measures of institutional power, we find that more power increases the number of requests that legislators make. Consider first the effect of being a committee chair. We estimate that becoming a committee chair causes an increase of 0.27 requests \textit{per agency} (95-percent confidence interval [0.09, 0.45]). Across all 90 agencies, this represents an increase of approximately 24 additional requests per year, 24.1\% of the average number of requests per year in our data. There is a smaller increase for individuals who become ranking members and those who join a Prestige Committee, though the increase is statistically significant for the prestige committee. Becoming a ranking member of a committee causes an increase of 0.15 contacts per agency while joining a prestige committee causes a 0.27 per agency increase in the number of contacts a legislator makes. Across all 90 agencies in these data, this represents an increase of approximately 14 and 24 additional requests per year, respectively. Given the overall average of 72 letters per year, these differences are substantial. 
