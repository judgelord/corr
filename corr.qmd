...
# number of agencies for inline code
n_agency <-  readLines(here::here("docs", "tables", "n_agencies")) |> as.numeric()
```
...
`{r} readLines(here::here("docs", "tables", "n2007-2020coded"))` (of our total  `{r} readLines(here::here("docs", "tables", "n2007-2020"))`)
requests as policy work or constituency service. Doing so also yields
many illuminating descriptives about modern legislator behavior. For example, over 80% of contacts are made on behalf of
...
and the *shifting priorities theory* of legislator behavior. Critically, the
...
on `{r} readLines(here::here("docs", "tables", "n2007-2020"))` instances of
members of Congress contacting
`r n_agency` federal
agencies from 2007 to 2020.[^3] These contacts include letters from an individual legislators,
committee chairs in their capacity as committee chairs, and groups of
legislators. We were able to hand-code `{r} readLines(here::here("docs", "tables", "n2007-2020coded"))` for whether they focus on constituency service or policy.
...
Online Supplemental Information provides procedures and replication code for converting the raw
...
our codebook (see Online Supplemental Information). We then trained Research Assistants
...
questions about legislator behavior in U.S. politics. These descriptive
...
agencies. Since 2016, Gini coefficients for the number of contacts per year for the
House and Senate range between 0.56 and 0.90, exceeding Gini coeficient for income inequality in every country on which the World Bank reported in 2023. @fig-contact1 shows the average
...
## The Effects of Experience and Institutional Power  {#sec-prestige}
...
requests to agencies. We focus on three measures of a
...
have higher rates of service. Committee chairs, ranking
members, and members of prestige committees provide substantially more service than other legislators. However, these
...
`r n_agency` agencies,
this represents an increase of approximately `r beta$chair*n_agency` additional requests per
...
The results in Table 2 show that when legislators acquire more power, they make more requests of agencies.  This increase occurs regardless of the increase in power that we measure, but it is largest for committee chairs.  This demonstrates that as legislators acquire resources in the institution, they deploy those resources to interact with the bureaucracy. 


## The Effect of Electing a New Representative {#sec-tenure_dist}

We now turn to a related question: how does the level of constituency
service to a district change after the election of a new representative?  As legislators acquire more power, they contact agencies more. In this section we show that legislators also increase their capacity as they gather experience in the institution.  Rather than examining changes in the number of requests by making
...
#| tbl-cap: "The Effect of Electing a New Representative on Requests to Federal Agencies"
#| out-width: 100%

load(here::here("models", "models_district.Rdata"))
modelsummary(models_district,
             center_rows = 14:26,
             notes = list("Robust standard errors in parentheses, clustered by district", "This table shows how contacts on behalof of a district varies across districts. Model 1 is a cross-sectional comparison excluding district fixed effects. The second column is a district x year difference in differences model. Column 3 is the difference in differences model subsetted to U.S. House districts only.  Column 3 shows the difference in differences model subsetted to U.S. Senate delegations only.")) |>
  # bold new member line  
    tinytable::style_tt(i = 12:13, bold = T, background = "yellow") 
```
...
districts with a new representative have 34.7 fewer
requests made on their behalf. The magnitude of this difference shrinks
for districts represented by legislators in their second year (29.33
...
new member taking over: electing a new member causes the number of requests behalf of a given district to decrease by about 40%, or `r -beta$new_member` letters per year
...
#### The Costs of Electoral Turnover

Taken together, our results demonstrate that new legislators provide
much less policy work and less constituency service. Legislators in their first year and second year contact federal agencies significantly less than legislators serving longer than six years. Further, when
districts elect a new representative or senator, people in the district experiences a sharp decrease in requests made on their behalf. Rather
...
## Experienced and Powerful Legislators Send Relatively Fewer Constituent Requests {#sec-priority}
...
to decrease by 0.07 (95-percent confidence interval [-0.05 , -0.12 ]). 
`r -beta$chair`(95-percent confidence interval [`r beta$chair-1.96*se$chair`, `r beta$chair+1.96*se$chair`]).
Becoming a ranking member of a committee causes a decrease of 0.03 in
the ratio.
...
sixth year). We estimate chair effects for our most senior category of
...
Taken together, these findings about how the ratio of policy work to constituency service varies across legislators show that as legislators acquire more experience and power their contacts with the bureaucracy become more policy focused. 

## Experienced and Powerful Legislators Maintain or Increase Levels of Constituency Service, While Also Performing a Higher Proportion of Policy Work

When legislators acquire capacity they make more requests of federal agencies.  At the same time, higher capacity legislators allocate a relatively larger share of their efforts towards policy work.  In this section we show that net result of these two simultaneous effects for in our time period (2007-2020) is that the total number of constituency service requests did not decrease, even as legislators allocated a relatively larger share of their attention towards policy.    

```{r}
load(here::here("models", "models_con.Rdata"))
```

To make this assessment, in Table 3 of the Supplementary Information we use our hand-coding of agency contacts to assess how legislators' levels of constituency service requests changes as their capacity changes.  Using the same difference-in-differences specification as in Model 2 of @tbl-models-total, we find that a given **legislator** who becomes committee chairperson makes an additional `r beta$chair` constituency service contacts per-agency per year, an increase that is not statistically significant at standard levels of confidence (95 percent confidence interval [`r beta$chair-se$chair*1.96`, `r beta$chair+se$chair*1.96`]).  

```{r}
load(here::here("models", "models_policy.Rdata"))
```

In contrast in Table 4 of the Supplementary Information we find using the same difference-in-difference model that becoming a committee chairperson causes a  `r beta$chair`  additional policy contacts per-agency year (95 percent confidence interval [`r beta$chair-se$chair*1.96`, `r beta$chair+se$chair*1.96`]), a statistically significant difference compared to the constituency service increase ([-0.00, 0.28]).^[We fail to detect an effect of becoming a ranking member or joining a prestige committee for constituency letter, while detecting an effect for policy letters.  That said, we fail to reject the null of a difference in the change in rates of constituency service and policy work for legislators who join prestige committees or become a ranking member.  We find first year legislators make fewer constituency service and policy work requests.]  

```{r}
load(here::here("models", "models_district_con.Rdata"))
```

Similarly, in Table 5 of the Supplementary Information, we use our hand-coding of agency contacts to assess how the level of constituency service requests coming from a given **district** changes when a new member is elected. Using the same difference-in-differences specification as in Model 2 of @tbl-models-district, we find that electing a new member causes a significant decrease in the level of constituency service. When a district replaces and experienced member with a new legislator, we estimate `r -beta$new_member` fewer constituency service contacts per year, an increase that is not statistically significant at standard levels of confidence (95 percent confidence interval [`r beta$new_member-se$new_member*1.96`, `r beta$new_member+se$new_member*1.96`]

In short, the "shifting priorities" effects that we find are insufficiently large to overpower the "increasing capacity" effects to cause an overall decrease constituency service among longer serving or more powerful legislators. Indeed, districts that replace an experienced legislator with a new legislator see a significant decrease in **both** policy work and constituency service for several years. Longer-serving legislators do more constituency service, not less. <!--We can see that from these two sets of analyses by recalling that in the difference-in-difference specification of our level analysis (Model 2 of Table 2) we found that becoming a committee chair causes an increase of 0.29 requests per agency (95-percent confidence interval [0.12,0.47]). Across all 92 agencies, this represents an increase of approximately 27 additional requests per year.--> <!--TODO INSERT EVIDENCE FOR THIS --> 

<!--However, as we discuss further in @sec-discussion, the total number of constituency service requests do not drop, even as the proportion of requests that are constituency service related decreases. -->
...
variation in constituency service discussed in @sec-prestige.
...
face validity for our measures of legislator behavior, but they also suggest
...
@sec-prestige, we measure the number of requests a district's
...
Columns 2 and 3 of @tbl-models-spillover that either a new member in the delgation or the proportion of new members in the delgation results in decreased constituency service (insigificant in model 2, significant in model 3) when focusing on senators only. This consistent null or negative finding
