...
explanations. @sec-discussion and @sec-conclude highlight the implications
...
policy work as they gain experience and power. The "Potomac fever" concern is that, as legislators gain experience and power, the magnitude
...
service due to *changes in capacity* and *shifting priorities*. The top-left cell of @tbl-theory shows our expectations if both mechanisms
...
|  **to policy work**       | -   $\Delta$ Ratio of $\frac{\textrm{policy}}{\text{service}}$: $\uparrow$ | -   $\Delta$ Ratio of $\frac{\textrm{policy}}{\text{service}}$: $\uparrow$ |
+---------------------------+----------------------------------------------------------------------------+----------------------------------------------------------------------------+
|                           |                                                                            |                                                                            |
+---------------------------+----------------------------------------------------------------------------+----------------------------------------------------------------------------+
| **No change**             | -   $\Delta$ Level of service: $\uparrow$                                  | -   $\Delta$ Level of service: $0$                                         |
| **in priorities**         | -   $\Delta$ Ratio of $\frac{\textrm{policy}}{\text{service}}$: $0$        | -   $\Delta$ Ratio of $\frac{\textrm{policy}}{\text{service}}$: $0$        |
+---------------------------+----------------------------------------------------------------------------+----------------------------------------------------------------------------+
...
`r n_agency` federal
agencies from 2007 to 2020.[^3] These contacts include letters from individual legislators,
committee chairs in their capacity as committee chairs, and groups of
legislators. We were able to hand-code `{r} readLines(here::here("docs", "tables", "n2007-2020coded"))` for whether they focus on constituency service or policy. Models discussed in @sec-capacity and @sec-district use the full sample. Models discussed in @sec-priority and @sec-joint use the hand-coded sample. 
...
always to provide constituency service, with only a small fraction focused on
...
@sec-capacity. We find that 76% of requests from committee chairs are
...
[^4]: To measure committee prestige, we use a revealed preference
    ...
    number of desirable committees. For House members, the "exclusive" committees are: Appropriations, Energy & Commerce, Financial
...
House and Senate range between 0.56 and 0.90, exceeding the Gini coefficient for income inequality in every country the World Bank reported in 2023. @fig-contact1 shows the average
number of contact rates per year for House members (left-hand panel) and
senators (right-hand panel).  Senator Robert Byrd (D-WV) averaged 538
contacts per year, Senator Bill Nelson (D-FL) averaged 535, and Senator James Webb (D-VA) averaged 533
...
fewer constituents than senators. Rep. Barbara Comstock averaged 349 contacts per year. Comstock replaced Rep. Frank Wolf (R-VA), who averaged 381 contacts per year and inherited many of his staff. Comstock had worked in Rep. Frank Wolf's congressional office for five years, described him as a "longtime mentor," and claimed that Wolf first urged her to run for office. He endorsed and campaigned with her. Nevertheless, Comstock's first year saw the volume of contacts on behalf of Virginia's 10th district dip slightly to 245, their lowest levels in our time period. 

Like the Senate, other members of the House wrote at
...
per year by the end of her time in Congress.--> House members
averaged 54 contacts with federal agencies per year. But like the
...
## The Effects of Experience and Institutional Power  {#sec-capacity}
...
#| tbl-cap: "The Effect of Institutional Power on Requests to Federal Agencies"
#| out-width: 100%
...
modelsummary(models_total,
  ...
  # bold new member line  
    tinytable::style_tt(i = 2:3, #bold = T, 
                        background = "yellow") 
```
...
Other measures of institutional power included in these models (ranking minority member and prestige committee member) are associated with similar increased levels of contact with federal agencies, but these effects are only significant at the .05 level in the cross-sectional and difference in differences design with the logged number of contacts as the dependent variable (Models 1 and 4). It is worth noting that because we estimate the effects of becoming a committee chair and being a member of a prestige committee in the same model, these effects are additive: prestige committee chairs may make more requests than non-prestige committee chairs. 

Our evidence about the effects of majority party status or having a co-partisan president is less conclusive. Majority party status is negatively associated with contacting agencies only in Model 4, predicting the logged number of contacts. Having a co-partisan president is negatively associated with contacting agencies on average but has a positive effect in the within-member analysis, which is only significant at the .05 level in Model 4.

```{r}
#| fig-cap: "Predicted Number of Total Requests to Agencies (Within Legislator Difference in Differences), 2007-2020"
...
bureaucracy fewer times than the Senate average (the dotted line in the bottom panel of @fig-wi) were Baldwin's first year and Johnson's first
...
[^7]: It is worth noting that this treatment is fundamentally different
    for a district than within-legislator variation. In each election, each district allows its incumbent to acquire another term or replaces them. This differs from within-legislator comparisons
...
modelsummary(models_district,
  ...
  # bold new member line  
    tinytable::style_tt(i = 12:13, #bold = T, 
                        background = "yellow") 
```
...
new member taking over: electing a new member causes the number of requests on behalf of a given district to decrease by about 40%, or `r -beta$new_member` letters per year
...
```{r}
#| fig-cap: "Predicted Number of Total Requests to Agencies Per District, 2007-2020"
...
[^8]: Predictions are based on a district-year pair where (1) the
    ...
    the district's number of contacts with the agency equals the average
    received by that agency, (3) and the agency received an average
    number of letters.

#### The Costs of Electoral Turnover

Taken together, our results demonstrate that a new legislator will make fewer requests to federal agencies than the more experienced legislator they replaced. Legislators in their first year and second year contact federal agencies significantly less than legislators serving longer than six years. Further, when districts elect a new representative or senator, people in the district experience a sharp decrease in requests made on their behalf. Rather
...
to decrease by `r -beta$chair` (95-percent confidence interval [`r beta$chair-1.96*se$chair`, `r beta$chair+1.96*se$chair`]).
<!--
...
## Experienced and Powerful Legislators Maintain or Increase Levels of Constituency Service, While Also Prioritizing Policy Work {#sec-joint}

When legislators acquire capacity they make more requests of federal agencies.  At the same time, higher capacity legislators allocate a relatively larger share of their efforts towards policy work.  In this section, we show that the net result of these two simultaneous effects in our time period (2007-2020) is that the total number of constituency service requests did not decrease, even as legislators allocated a relatively larger share of their attention towards policy.    
...
In short, the "shifting priorities" effects that we find are insufficiently large to overpower the "increasing capacity" effects to cause an overall decrease in constituency service among longer-serving or more powerful legislators. Indeed, districts that replace an experienced legislator with a new legislator see a significant **decrease in both policy work and constituency service** for several years. Longer-serving legislators do more constituency service, not less. <!--We can see that from these two sets of analyses by recalling that in the difference-in-difference specification of our level analysis (Model 2 of Table 2) we found that becoming a committee chair causes an increase of 0.29 requests per agency (95 percent confidence interval [0.12,0.47]). Across all 92 agencies, this represents an increase of approximately 27 additional requests per year.
--> <!--TODO INSERT EVIDENCE FOR THIS --> 

<!--However, as we discuss further in @sec-discussion, the total number of constituency service requests does not drop, even as the proportion of requests that are constituency service-related decreases. -->
...
shifts do not appear to explain the within-legislator or within-district variation we observe with changing committee assignments
...
variation in constituency service discussed in @sec-capacity.
...
overall number of requests and that constituency characteristics---the proportion of veterans and the proportion over 65---correlate with the
...
our empirical strategies in @sec-capacity and @sec-priority account for
...
constituency service of experienced members are driven by higher
constituent demand.
...
in their state.  Because House members generally do not provide constituency service to people outside their district, we measure
the effect on the House member of a new Senator in the state delegation. For Senators, we measure new members in the state in two ways: either the proportion of a
state delegation that is new or an indicator of whether there is a new
legislator (House or Senate) in the delegation. As in
@sec-capacity, we measure the number of requests a district's
...
#| tbl-cap: "Spillover Redirected to Expierenced Legislators when a New Legislator is Elected"
#| out-width: 100%
...
modelsummary(models_spillover,  
             ...
             notes = list("Robust standard errors in parentheses, clustered by district",
                          "The first column estimates the effect of an experienced Senator being replaced by a new Senator on the level of constituency service that other legislators in the state provide. Columns 2 and 3 show the effect of a new member of a delegation or the portion of new members of a delegation on senators from that delegation.")) 
```
...
Columns 2 and 3 of @tbl-models-spillover show, respectively, that either a new member in the delegation or the proportion of new members in the delegation results in decreased constituency service (insignificant in model 2, significant in model 3) when focusing on senators only. This consistent null or negative finding regarding the impact of new legislators in a state delegation provides reassurance that there is no evidence of constituents redirecting demand toward other, more experienced legislators in response to having new representatives in their state delegation.
...
organizational efficiency mechanism. Becoming a com mittee chair
...
constituency service is a core function of congressional offices. This
...
these questions. With data from nearly all parts of the vast U.S. federal bureaucracy, future work can advance the study of descriptive
