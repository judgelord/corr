...
format:
  #html
  #  docx # sent to AJPS for typsetting 
  #pdf # basic pdf 
  cmc-article-pdf: # the PDF manuscript uploaded applies additional formatting from  https://github.com/CoryMcCartan/cmc-article thanks to Cory McCartan and Christopher Kenny
...
dataverse: "https://doi.org/10.7910/DVN/LWOCW"
...
series of additional analyses, we find little evidence
that constituents shift demands to more established legislators when a new representative is elected. 
...
How elected officials balance their work on broad policy goals and particularistic service to their constituents presents a significant tension for representation. Building on multi-task models of representation [@AshworthBuenodeMesquita2006; @gordon2009advantages], we explain why increasing experience and power may either increase or decrease the levels of constituency service legislators provide depending on the relative magnitudes of the effects of increasing capacity and changing priorities.
...
regulate compliance with environmental, health, and
...
less on the particular demands of their constituents. 
...
```{r}
#| fig-cap: "The countervailing effects of increasing capacity and shifting priorities on constituency service. Note: These figures   show potential changes in the absolute volume of constituency service and the ratio of constituency service to policy work due to changes in capacity and shifting priorities. (a) shows our expectations if capacity increases but priorities remain unchanged. (b) shows our expectations if priorities change and capacity remains unchanged (c) shows a possible outcome if both mechanisms affect behavior and the magnitude of the effect of increased capacity is large enough to overcome the countervailing effect of shifting priorities (any outcome in the upper, darker-shaded region of (d))."
#| label: fig-2x2
#| fig-subcap: 
#|   - "Increasing capacity"
#|   - "Shifting priorities"
...
knitr::include_graphics("figs/2x2-fig-1.png")
knitr::include_graphics("figs/2x2-fig-2.png")
knitr::include_graphics("figs/2x2-fig-3.png")
knitr::include_graphics("figs/2x2-fig-4.png")
```
...
targeted at the more experienced legislators of a delegation whenever a less
...
constituency demand-driven explanations in @sec-demand. We find little evidence of requests
...
[^2]: In addition to our initial requests, collecting these data
    ...
    Quality and the U.S. Trade Representative), and pseudo-governmental
...
our codebook (see Supplemental Information, p. 2). We then trained research assistants
to continue the coding. The first several thousand letters were
double-coded. For example, of over 10,000 letters to the Environmental
...
constituents navigate the federal bureaucracy.--> @fig-by-type shows the
...
```{r}
#| fig-cap: "Legislator requests to federal agencies by type, 2007-2020. Note: The center bar shows that 71% of all legislator requests to federal agencies are on behalf of individual constituents. Requests on behalf of individual corporations are a smaller percentage, 6%. 7% of requests are on behalf of nonprofits and local governments. General policy work and policy work on behalf of specific industries account for just 16% of all requests made to federal agencies."
#| label: fig-by-type
#| out-width: 80%

knitr::include_graphics("figs/data_by_type-1.png")
```
...
agencies. Since 2016, Gini coefficients for inequality in the number of contacts per year per legislator range between 0.49 and 0.54, mirroring the Gini coefficient for income inequality in the eight most unequal countries reported by the World Bank in 2024. To visualize this variation, @fig-percentiles shows the average number of contacts per year for House members (panel (a)) and
senators (panel (b)).[^5] 
On average, senators in our data contacted agencies
...
```{r}
#| fig-cap: "Variation in average legislator requests by percentile. Note: This figure shows that some legislators contact agencies many times per year, while many others do so rarely. We see these highly unequal rates of contacting agencies in both the House and Senate."
...
knitr::include_graphics("figs/percentiles-chamber-1.png")
knitr::include_graphics("figs/percentiles-chamber-2.png")
```
...
Nevertheless, Comstock's first year saw the volume of contacts on behalf of Virginia's 10th district dip slightly to 245 in her first year before rebounding to to 332 in her second year. Other members of the House wrote at
...
[^5]: Figure A1 in the Supplemental Appendix (p.6) shows the number of requests per legislator over time.
...
$\sum_{s = 1}^{6} \eta_{s} \text{tenure}_{it}$ as controls. 
<!--This
...
load(here::here("models", "models_total-manuscript-table-2.Rdata"))

modelsummary(models_total,
             center_rows = 23:29,
             notes = list("Note: Robust standard errors in parentheses, clustered by legislator.",
                          "This table shows how the number of requests changes as legislators acquire power. Column 1 shows average differences across committee positions and years in Congress. Column 2 presents difference-in-differences estimates. Column 3 subsets to legislators who serve at least three years. Column 4 makes the Log of the counts + 1 the dependent variable.
                          All coefficients represent average additional requests per year per agency per legislator. Legislator per year effects are simply these coefficients times the number of agencies in the data.")) 
```
...
```{r}
#| fig-cap: "Predicted number of total requests to federal agencies (within legislator difference in differences), 2007-2020. Note: This figure shows the predicted total number of letters per year by whether a legislator is a member of the president's party (subfigure a) and whether they are a chair (subfigure b, comparing predictions for counterfactuals where the same legislator did and did not receive a chair position in their sixth year)"
#| label: fig-m-total-predicted
#| fig-subcap:
#|   - "Predicted requests by presidents' party"
#|   - "Predicted requests by years of service in Congress"
#| layout-ncol: 2
#| out-width: 100%

knitr::include_graphics("figs/m-total-predicted-3.png")
...
```{r}
#| fig-cap: "The effect of electing new legislators in Wisconsin on total requests to federal agencies. Note: This figure shows the change in contacts from legislators representing Wisconsin's 7th district in the House (top) and the Senate (bottom)."
#| fig-subcap: 
#|   - "Representatives of Wisconsin's 7th District"
#|   - "Senators from Wisconsin"
#| label: fig-wi
#| out-width: 80%

knitr::include_graphics("figs/cases-2.png")
knitr::include_graphics("figs/cases-1.png")
...
load(here::here("models", "models_district-manuscript-table-3.Rdata"))
modelsummary(models_district,
             center_rows = 14:26,
             notes = list("Note: Robust standard errors in parentheses, clustered by district", 
                          "This table shows how contacts on behalf of a district vary across districts. Model 1 is a cross-sectional comparison excluding district fixed effects. The second column is a district x year difference in differences model. Column 3 is the difference in differences model subsetted to U.S. House districts only.  Column 4 shows the difference in differences model subsetted to U.S. Senate delegations only.")) 

beta_cross <- round(beta_cross)
beta <- round(beta)
```
...
districts with a new representative have  `r -beta_cross$new_member` fewer
requests made on their behalf. The magnitude of this difference shrinks
for districts represented by legislators in their second year (`r -beta_cross$second`
...
new member taking over: electing a new member causes the number of requests on behalf of a given district to **decrease by `r percent(-beta$new_member/84)` **, or `r -beta$new_member` letters per year
...
```{r}
#| fig-cap: "Predicted number of total requests to federal agencies per district, 2007-2020. Note: These plots show that, across all models, electing a new member is associated with a decrease in the number of requests on behalf of a given district."
#| label: fig-m-district-predicted
#| fig-subcap: 
#|   - "Model 1: Cross-sectional"
#|   - "Model 2: Within-legislator"
#|   - "Model 3: Within-legislator (House)"
#|   - "Model 4: Within-legislator (Senate)"
#| layout-ncol: 2
#| out-width: 100%

knitr::include_graphics("figs/m-district-predicted-1.png")
knitr::include_graphics("figs/m-district-predicted-3.png")
knitr::include_graphics("figs/m-district-predicted-house-1.png")
knitr::include_graphics("figs/m-district-predicted-senate-1.png")
...
load(here::here("models", "models_ratio-manuscript-table-4.Rdata"))
...
#| fig-cap: "Predicted ratio of constituency service to policy work (within legislator difference in differences), 2007-2020. Note: This figure shows the predicted ratio of constituency service to policy work. Subfigure (a) shows predictions by committee chair status and whether that member has a co-partisan president. Subfigure (b) shows predictions for counterfactuals where the same legislator did and did not receive a chair position in their sixth year."
#| fig-subcap: 
...
knitr::include_graphics("figs/m-ratio-predicted-3.png")
knitr::include_graphics("figs/m-ratio-predicted-4.png")
...
In Table A3 of the Supplementary Information (p. 11), we use our hand-coding of agency contacts to assess how legislators' levels of constituency service requests change as their capacity changes.  Using the same difference-in-differences specification as in Model 2 of @tbl-models-total, we find that a given **legislator** who becomes committee chair makes an additional `r beta$chair ` constituency service contacts per agency per year, an increase that is not statistically significant (95 percent confidence interval [`r beta$chair-se$chair*1.96`, `r beta$chair+se$chair*1.96`]).  All models in Table A4 of the Supplementary Information (p. 13) show significant increases in policy work as a legislator gains experience or a committee leadership position.
...
We find that population size [@Census2019] correlates with the overall number of legislator requests. 
...
larger states contact federal agencies more often on average. 
...
```{r}
#| fig-cap: "Average number of total requests per senator per year 2007-2020 by state population. Note: This figure  shows that senators from larger states contact federal agencies more often on average."
#| label: fig-stateSize
#| out-width: 70%

knitr::include_graphics("figs/pop-1.png")
```

## Do Voters Demand More of More Powerful Legislators?

Can variation in demand for constituency service explain why legislators increase contacts with agencies when they gain experience and committee leadership positions?
...
representative makes in a particular year. Since we aim to test whether constituent demand drives our results, @tbl-models-spillover shows estimates using requests hand-coded as constituency service. The results for total requests are nearly identical (see Supplemental Information p. 21).  
...
load(here::here("models", "models_spillover_con-manuscript-table-5.Rdata"))
...
incumbent legislators from the state shows a marginally significant impact (p<0.1) on the level of constituency service that other legislators provide.
Columns 2 and 3 of @tbl-models-spillover focus only on Senators. They show that either a new member in the delegation or the proportion of new members does not increase incumbents' constituency service (indeed, we see a statistically significant decrease in model 3). The results are inconsistent, ranging from weak positive (Model 1), null (Model 2), and statistically significant negative (Model 3) across model specifications, which provides reassurance that there is little evidence of constituents redirecting demand toward other, more experienced legislators in response to having new representatives in their state delegation.
