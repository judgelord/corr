...
decrease the number of requests they make to agencies. Newly-elected legislators
must hire staff, open district offices, and establish
priorities and procedures in their office for handling constituency
service requests. They lack the standard procedures that more
...
capacity [@AshworthBuenodeMesquita2006]. These start-up costs should subside as
...
In fulfilling statutory missions, agencies use broad discretion to prioritize resources, for example in processing visa, permit, and grant applications,
...
with legislator requests may help agencies achieve their own goals. If agency staff aim to grow thier coalition of political supporters, we expect
...
## Shifting Priorities: Why Experience and Power Could Decrease Legislators' Constituency Service Efforts
...
service to stay the same or even increase as legislators gain experience and
...
In short, the "shifting priorities" effects that we find are insufficiently large to overpower the "increasing capacity" effects to cause an overall decrease in constituency service among longer-serving or more powerful legislators. Indeed, districts that replace an experienced legislator with a new legislator see a significant **decrease in both policy work and constituency service** for several years. As expected by theories focusing on both capacity and priorities, more experienced and powerful legislators do significantly more policy work. (See models with policy-work-only sample in the Supplemental Information.) We also find support for both theories. Interestingly the capacity effects overcome the effects of shifting priorities, meaning that longer-serving legislators do more constituency service, not less. <!--We can see that from these two sets of analyses by recalling that in the difference-in-difference specification of our level analysis (Model 2 of Table 2) we found that becoming a committee chair causes an increase of 0.29 requests per agency (95 percent confidence interval [0.12,0.47]). Across all 92 agencies, this represents an increase of approximately 27 additional requests per year.
...
the effect of a new Senator in a House member's state delegation. For Senators, we measure new members in the state in two ways: either the proportion of a
...
representative makes in a particular year. Since we aim to test whether effects are driven by constituent demand, @tbl-models-spillover shows estimates using requests hand coded as constituency service, but the results are nearly identical for total requests (see Supplemental Information).  Using district and year fixed effects, we estimate a series of difference-in-differences regressions
...
service requests to more experienced members, restrict the regression
to incumbent legislators.

```{r}
#| label: tbl-models-spillover
#| tbl-cap: "Spillover Effects: Constituent Requests Redirected to Expierenced Legislators when a New Legislator is Elected"
#| out-width: 100%
...
modelsummary(models_spillover_con,  
             ...
             notes = list("Robust standard errors in parentheses, clustered by district",
                          "The first column estimates the effect of an experienced Senator being replaced by a new Senator on the level of constituent service that other legislators in the state provide (total requests to federal agencies per year). Columns 2 and 3 show the effect of a new member of a delegation or the portion of new members of a delegation on senators from that delegation.")) 
```
...
At the same time, we document surprising levels of inequity in the levels of both service and policy that members provide. 
This variation is even greater than can be explained by the substantively large effects of experience and power, suggesting that the "service-policy divide" [@ButlerKarpowitzPope2012] offers an incomplete picture of how legislators use their capacity. Future work could investigate how experience and institutional power affect other uses of staff resources, such as communications and fundraising, that do not require contacting federal agencies and are thus outside the scope of our data. 
