...
agencies. Since 2016, Gini coefficients for inequality in the number of contacts per year among members of each chamber range between 0.56 and 0.90, exceeding the Gini coefficient for income inequality in every country the World Bank reported in 2023. To visualize this variation, @fig-percentiles shows the average number of contacts per year for House members (panel (a)) and
senators (panel (b)).[^5] 
...
modelsummary(models_district,
             ...
             notes = list("Note: Robust standard errors in parentheses, clustered by district", 
                          "This table shows how contacts on behalf of a district vary across districts. Model 1 is a cross-sectional comparison excluding district fixed effects. The second column is a district x year difference in differences model. Column 3 is the difference in differences model subsetted to U.S. House districts only.  Column 3 shows the difference in differences model subsetted to U.S. Senate delegations only.")) 
...
districts with a new representative have 34 fewer
requests made on their behalf. The magnitude of this difference shrinks
for districts represented by legislators in their second year (30
...
new member taking over: electing a new member causes the number of requests on behalf of a given district to **decrease by `r percent(-beta$new_member/84)` **, or `r -beta$new_member |> round()` letters per year
